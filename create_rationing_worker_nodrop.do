...
/* This do file contains code that generates rationing_worker_nodrop.dta, which is used in reproducing Appendix Table 13 in "Labor Rationing" (Breza, Kaur & Shamdasani) */
	
use "./data/rawdata/rationing_worker_raw_nodrop.dta", clear 
...
* peak variable	
	egen temp = mean(e_manwage) if surv_el==1 & sample_signup==1 & sample_hiringshock==0 & treat==0 & survey_not_complete==0 & round_id~=14 & round_id~=23, by(month_el)
	egen avg_emanwage_ctrl = max(temp), by(round_id)
	sum avg_emanwage_ctrl if round1==1 & round_id~=14 & round_id~=23, det
